...
export default {
  async fetch(request, env) {
    ...
    // Public endpoints:
    // - GET /admin/config: returns booleans about which keys are configured
    //   (never the keys themselves)
    // - POST /tts: voice narration (used in-app, no auth required for the
    //   audio fetch itself since the request originates from a logged-in UI)
    ...
    // ── Watchtower watchlist (operational dependencies referenced by Regulatory Scans) ──
...
// Lightweight retrieval helper: discovers all file records in the library
// and scans their embedded `chunks` arrays for keyword hits. Used by
// regulatory scan plan + run to find related context across the library.
//
...
// ═══════════════════════════════════════════════════════════════
// WATCHTOWER — Operational watchlist management
// ═══════════════════════════════════════════════════════════════
// Data model:
//   wt:watchlist                     → JSON array of operational dependency entries
// (Per-document analysis was migrated to /regscan/* — this section now only
// holds the operational watchlist that the regscan prompt builder reads.)
...
- whatChanged (THE CHANGE VECTOR — see CHANGE VECTOR FORMAT below)
...
- supersedesOrUpdates (string description of what this replaces — e.g. "supersedes the previous two-driver transportation requirement" or "replaces O.A.C. 3796 packaging rules" — null ONLY if this is a genuinely standalone new rule with no prior counterpart)

CHANGE VECTOR FORMAT — whatChanged must contrast OLD STATE against NEW STATE so a reader can see what shifted, not just what's required now.

Write whatChanged as 2-4 sentences in this structure:
  "Before [date or rule citation]: [the prior rule or practice]. Now/As of [date]: [the new rule]. [Optional third sentence: the operational delta — what specifically must change in practice.]"

GOOD EXAMPLES (do this):
  ✓ "Before April 21, 2026: a single master METRC API key was permitted for dispensary POS integrations. As of April 21, 2026: every dispensary employee must hold their own METRC API key by May 15, 2026, and key sharing is prohibited. POS systems configured against a master key must be reconfigured to per-employee keys."
  ✓ "Before this guidance: licensees could independently quarantine, remediate, relabel, or destroy non-compliant products at their discretion. As of February 26, 2026: any non-compliant product must be quarantined and reported to DCC within 24 hours via eLicense Incident Report, and no remediation, relabeling, transfer, or destruction may occur without DCC authorization."
  ✓ "Before October 17, 2025: THC potency was capped below 90% for licensed processors. As of October 17, 2025: processors may manufacture and register products up to 90% THC. Existing products needing the higher potency require new Item Brand and Product Item ID, but no new product concept submission."

BAD EXAMPLES (do not do this):
  ✗ "All dispensary employees must be registered with individual METRC API keys by May 15, 2026." [States only the new rule. Reader cannot tell what changed.]
  ✗ "Immediate quarantine and 24-hour DCC notification required for any non-compliant products." [States current obligation. Doesn't say what was permitted before.]
  ✗ "Licensed processors can now manufacture products with THC potency up to 90%." [The word "now" implies a change but never names the prior cap.]

If the document genuinely contains nothing operationally relevant, return { "findings": [] }.

THIN DOCUMENTS — short bulletins are still real documents. If the document body is only a paragraph or two, treat it as the source of truth for whatever it does say. Extract the change from the body text directly. A short DCC bulletin announcing a payment rounding rule is a real finding with whatChanged = "Before [date]: [prior practice]. As of [date]: [the rounding rule from the body text]." — even if both halves are brief. Length is not a reason to skip.

NEVER write findings using phrases like "Document summary not provided", "filename indicates", "this appears to address", "[Note: ...]", or any other pattern that signals you guessed from the title. If you can read the body, use the body. If you genuinely cannot read the body — meaning the body is empty, gibberish, or fewer than ~30 words of substantive content — return { "findings": [] } for that document. The system will report it as a skip rather than producing a low-quality finding.

If you genuinely cannot identify the prior state from the document text or the related older regulations provided as context, write whatChanged as: "[Prior state not stated in source materials.] As of [effective date]: [the new rule]." — but only after a real attempt. Most regulatory updates either name what they replace or imply it through language like "updated", "supersedes", "replaces", "rescinds", or "amended".
...
- supersedesOrUpdates and whatChanged work together — supersedesOrUpdates names the old rule, whatChanged shows the shift.
- Be specific in department instructions. "Verify month-end COGS calculations use unit-of-measure X" not "review this."
- Be honest. Don't invent findings to seem thorough. Don't compress multi-topic content into one finding. Don't fabricate prior states you can't substantiate from the source materials. Don't fabricate findings from filenames when document content is thin — return an empty findings array instead.`;
